	...
	private static HttpRes sendPostMsg(AbstractReq msg, int sendCount, int maxSendCount) {
		...
		if (NetworkCheckUitl.isOnline()) {
			...
			try {
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：POST消息第", sendCount, "次发送，Request URI：", uri,
						"，Token：", msg.getAccessToken(), "，msgContent：", msgContent);
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：POST消息响应，statusCode：", httpRes.getStatusCode(),
	...
	private static HttpRes sendGetMsg(String serverUrl, AbstractReq msg, int sendCount, int maxSendCount) {
		...
		if (NetworkCheckUitl.isOnline()) {
			...
			try {
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：GET消息第", sendCount, "次发送，Request URI：", uri,
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：GET消息响应，statusCode：", httpRes.getStatusCode(),
						"，content：", httpRes.getContent(), "，requestUri：", msg.getReuqestUri());
				return httpRes;
			} catch (Exception e) {
	...
	private static HttpRes sendPutMsg(AbstractReq msg, int sendCount, int maxSendCount) {
		...
		if (NetworkCheckUitl.isOnline()) {
			...
			try {
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：PUT消息第", sendCount, "次发送，Request URI：", uri,
						"，Token：", msg.getAccessToken(), "，msgContent：", msgContent);
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：PUT消息响应，statusCode：", httpRes.getStatusCode(),
						"，content：", httpRes.getContent(), "，requestUri：", msg.getReuqestUri());
				return httpRes;
			} catch (Exception e) {
	...
	private static HttpRes sendDeleteMsg(AbstractReq msg, int sendCount, int maxSendCount) {
		...
		if (NetworkCheckUitl.isOnline()) {
			...
			try {
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：DELETE消息第", sendCount, "次发送，Request URI：", uri,
						"，Token：", msg.getAccessToken());
				...
				Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：DELETE消息响应，statusCode：", httpRes.getStatusCode(),
						"，content：", httpRes.getContent(), "，requestUri：", msg.getReuqestUri());
				return httpRes;
			} catch (Exception e) {
